...
# Deploy script for Prime Intuition
# Usage: ./deploy-primeintuition.sh [staging|prod]

set -e

ENV=${1:-staging}
APP_NAME="primeintuition"
CONFIG_FILE="fly.primeintuition.toml"
ORG="staging-509"
DOMAIN="primeintuition.ai"

echo "🚀 Deploying Prime Intuition ($ENV) to org $ORG..."
...
if ! fly volumes list --app "$APP_NAME" | grep -q "primeintuition_data"; then
    echo "💾 Creating data volume..."
    fly volumes create primeintuition_data --size 1 --region sjc --app "$APP_NAME" --yes
fi
